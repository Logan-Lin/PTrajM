...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, B=None, C=None, dt=None, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        if B is not None:
            have_BCdt = True
            dstate = B.shape[-1] // ngroups
            d_nonssm = (zxbcdt.shape[-1] - 2 * dim) // 2
            assert d_nonssm >= 0
            assert zxbcdt.shape == (batch, seqlen, 2 * d_nonssm + 2 * dim)  
        else:
            have_BCdt = False
    ...
    @custom_bwd
    def backward(ctx, dout, *args):
        zxbcdt, conv1d_weight, conv1d_bias, out, A, D, B, C, dt, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight, outproj_bias = ctx.saved_tensors
        dfinal_states = args[0] if ctx.return_final_states else None
        headdim = ctx.headdim
        nheads = D.shape[0]
        dim = nheads * headdim
        ...
        return dzxbcdt, dweight, dbias, ddt_bias, dA, dD, None, \
                dB if ctx.have_BCdt else None, dC if ctx.have_BCdt else None, ddt if ctx.have_BCdt else None, \
                dinitial_states, None, None, None, None, drmsnorm_weight, None, doutproj_weight, doutproj_bias, None, None, None


def mamba_split_conv1d_scan_combined(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size,  B=None, C=None, dt=None, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    """
    Argument:
        ...
        A: (nheads)
        D: (nheads, headdim) or (nheads,)
        B/C: None or (B, L, ngroups * dstate)
        dt: None or (B, L, nheads)
        initial_states: (batch, nheads, headdim, dstate)
    ...
    Return:
        out: (batch, seqlen, dim)
    """
    return MambaSplitConv1dScanCombinedFn.apply(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, B, C, dt, initial_states, seq_idx, dt_limit, return_final_states, activation, rmsnorm_weight, rmsnorm_eps, outproj_weight, outproj_bias, headdim, ngroups, norm_before_gate)
